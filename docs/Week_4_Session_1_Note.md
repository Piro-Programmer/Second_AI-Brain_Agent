...
## Goal: Build an end-to-end system where you can capture anything (a note, a link, a file), have AI automatically classify and file it, auto-link it to related knowledge, render it as a live interactive graph you can explore, and — most importantly — ask it any question in plain English and get an

answer synthesized from your own accumulated knowledge. Then deploy it to a public URL anyone can open.
...
secondself/ ├── raw/ ├── wiki/ ├── capture.py ├── classify.py ├── link.py

\# Week 1: raw captures (timestamp + unique ID) # Week 2: classified + auto-linked notes # Week 1: one-command capture # Week 2: PARA classification via LLM # Week 2: embeddings + auto-linking

├── build_graph.py # Week 3: nodes/edges → graph.json

├── graph.json ├── ask.py ├── app.py ├── requirements.txt └── README.md

\# Week 3: exported graph data # Week 4: retrieval + LLM answer # Week 4: Streamlit UI (graph + search)
...
- [\- https://devin.ai/download/](https://devin.ai/download/)

- \- VS code + Claude

Obsidian link - https://obsidian.md/download [URL 🔗](https://obsidian.md/download)
...
- 9. Implement Sub-Phase 2.2 using the @docs/Implementation-plan.md

- 10. I want to view embeddings that are stored in data folder file name - embeddings.pkl

- Write a script to show me embeddings in the file

- 11. Implement Sub-Phase 3.1 as per the @docs/Implementation-plan.md

- 12. Implement Sub-Phase 3.2 as per the @docs/Implementation-plan.md

- 13. stop the project

- 14. Implement Sub-Phase 4.1 as per the @docs/Implementation-plan.md

- 15. Implement Sub-Phase 4.2 as per the @docs/Implementation-plan.md Deployment changes we can do in next week currently all local changes we can do [URL 🔗](http://implementation-plan.md)
...
| --- | --- | --- |
| capture_note(text) | Plain text / | CaptureResult |
|   | markdown |   |
|   | string |   |
| capture_link(url, notes="") | URL + optional | CaptureResult |
|   | notes |   |
...
python pipeline.py process find wiki/ -name "*.md" | wc -l # ≥ 15
...
| edges |   |
| Deduplica | Edge key = (min(source,target), max(source,target)) |
| te |   |
...
| --- | --- |
| on |   |
| Hover | Tooltip with note summary |
| node |   |
| Drag | Node moves, graph re-settles |
| node |   |
| Scroll | Zoom in/out |
| Click | (Optional) highlight connected edges |
| node |   |
...
st.components.v1.html() embedding vis-network

capture.capture_note(text)

pipeline.process() → rebuild graph

Count nodes/edges from graph.json
...
- 1. Connect GitHub repo at share.streamlit.io [URL 🔗](https://share.streamlit.io/)
...
| Risk | Phas | Mitigation |
| --- | --- | --- |
|   | e |   |
| Groq API rate limits | 2, 4 | Batch classify; add retry with backoff |
| Embedding model slow on | 2, 4 | @st.cache_resource in Streamlit |
| first load |   |   |
...
\# Phase 1 python capture.py note "..." python capture.py link "https://..." python capture.py file ./doc.pdf
